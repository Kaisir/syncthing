...
RUN apt-get update && apt-get install -y ca-certificates --no-install-recommends && apt-get clean all && rm -rf /var/lib/apt/lists/*
...
RUN set -x \
	&& apt-get update && apt-get install -y curl --no-install-recommends && apt-get clean all && rm -rf /var/lib/apt/lists/* \
